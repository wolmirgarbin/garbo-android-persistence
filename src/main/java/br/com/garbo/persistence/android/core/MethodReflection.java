...
public final class MethodReflection {
	...
	private MethodReflection(Class<?> clazz) {
		this.clazz = clazz;
	}
	...
	public final Field returnField(String fieldName) {
		try {
			...
			throw new GarboPersistenceException(MessageFormat.format(bundle.getString("field_not_found"), fieldName, clazz.getName()));
		}
	}
	...
	public final Method returnSetMethodOf(Field field) {
		...
		try {
			...
			throw new GarboPersistenceException(MessageFormat.format(bundle.getString("method_not_found"), methodName.toString(), clazz.getName()));
		}
	}
	...
	public final Method returnMethod(String methodName, Class<?>... paramTypes) {
		try {
			...
			throw new GarboPersistenceException(MessageFormat.format(bundle.getString("method_not_found"), methodName, clazz.getName()));
		}
	}
	...
	public final static MethodReflection in(Class<?> clazz) {
		return new MethodReflection(clazz);
	}
	...
	public final static Object getFieldValue(Object of, Field field) {
		try {
			...
			throw new GarboPersistenceException(MessageFormat.format(bundle.getString("get_field_value_error"), field.getName(), e.getMessage()));
		}
	}
	...
	public final static <T> T newInstance(Class<T> clazz) {
		try {
			...
			throw new GarboPersistenceException(MessageFormat.format(bundle.getString("object_creation_error"), clazz.getCanonicalName()));
